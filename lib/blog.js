  ...
  return blogDirs.map(slug => {
    ...
    if (data.cover) {
      ...
      if (data.cover.startsWith('./')) {
        ...
        const imageName = data.cover.substring(2); // Remove './'
        coverImagePath = `/blog-images/${slug}/${imageName}`;
      } else if (data.cover.startsWith('/')) {
        // Absolute path
        coverImagePath = data.cover;
      } else {
        // Assume it's a filename in the blog directory
        coverImagePath = `/blog-images/${slug}/${data.cover}`;
      }
    } else {
      ...
      if (coverImage) {
        coverImagePath = `/blog-images/${slug}/${coverImage}`;
      }
    }
