...
export function getAllBlogPosts() {
  ...
  return blogDirs.map(slug => {
    let indexPath = path.join(contentDirectory, slug, 'index.md');
    
    if (!fs.existsSync(indexPath)) {
      indexPath = path.join(contentDirectory, slug, 'index.mdx');
      if (!fs.existsSync(indexPath)) {
        throw new Error(`index.md or index.mdx not found for blog: ${slug}`);
      }
    }
